...
def get_dataset(dataset=['train'], maximum_steps=None, batch_size=16, mode='training', weights=None, split='train', val_ratio=0.1):
    ...
    if mode == 'training':
        ...
        ds = tf.data.Dataset.sample_from_datasets([phos_ds, other_ds], weights)
    elif mode == 'test':
        if weights is None: weights = [0.5, 0.5]
        ds = tf.data.Dataset.sample_from_datasets([phos_ds, other_ds], weights, seed=42)
    elif mode == 'inference':
...
def get_dataset_inference(mgf_file='example.mgf',batch_size=16):
    ...
    with mgf.chain.from_iterable([mgf_file]) as mgf_reader:
        ds = tf.data.Dataset.from_generator(
            lambda: generator(label=None,reader=mgf_reader),
            output_signature=(
                tf.TensorSpec(shape=(), dtype=tf.float32),
                tf.TensorSpec(shape=(None,), dtype=tf.float32),
                tf.TensorSpec(shape=(None,), dtype=tf.float32),
